...
// Generated 19-Jun-2022 18:16:21
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"microaccel\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(microaccel_M, (NULL));
        microaccel_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(microaccel_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(microaccel_M)
                == (NULL)) || rtmGetStopRequested(microaccel_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(microaccel_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
